...
def dyd_m_SBP_periodic(uy, u, ny, dy, order):
    # summation-by-parts finite difference operators for first derivatives du/dx
    # summation-by-parts finite difference operators for first derivatives du/dx
    
    m = ny-1
    
    # second orders accurate case
    if order==2:
        # calculate partial derivatives on the boundaries:[0, m] using periodicity
        uy[:, 0] = (u[1, :] -  u[m, :])/dy
        uy[:, m] = (u[0, :] -  u[m-1, :])/dy
    ...
    if order==4:
        ...
        for i in range(ny):
            uy[:,i] = 0.083333333333333*u[:,(i-2)%m] - 0.666666666666667*u[:,(i-1)%m] + 0.666666666666667*u[:,(i+1)%m] - 0.083333333333333*u[:,(i+2)%m]
        uy[:,:] = uy[:,:]/dy
    ...
    if order==6:
        ...
        for i in range(m):
            uy[:,1] = -0.016666666666667*u[(i-3)%m,:] + 0.15*u[(i-2)%m,:] - 0.75*u[(i-1)%m,:] + 0.75*u[(i+1)%m,:] - 0.15*u[(i+2)%m,:] + 0.016666666666667*u[(i+3)%m,:]
